...
class StatFuncs {
  StatFuncs();
  List<int> sortedTimes = Hive.box<Solve>(solveBox).values.map((solve) => solve.time.inMilliseconds).toList()..sort();
  
  String? aox(int timeCount){
    final lastXTimes = sortedTimes.sublist(sortedTimes.length - timeCount);
    String? aoX; 

    final averageMs = (lastXTimes.reduce((a, b) => a + b) / lastXTimes.length.toDouble());
  ...
  String mediaBruta(){
    final averageMs = (sortedTimes.reduce((a, b) => a + b) / sortedTimes.length.toDouble());
    String? average; 
    final duration  = Duration(milliseconds: averageMs.round());
    String twoDigits(int n) => n.toString().padLeft(2, '0');
    String threeDigits(int n) => n.toString().padLeft(3,'0');
  ...
  String desviacionEstandar(){
    ...
    double media = (sortedTimes.reduce((a, b) => a + b) / sortedTimes.length.toDouble());
    double sum=0;
    String desviacion;
